...
we observe that some combinations cease to exist in our data; e.g. we never observe Charlotte Bobcats play Utah Jazz on the road and then fly directly back home for a game. Lastly, notice the patterns exhibited when last game is played on the road, and current game also played on the road. We see that when last game location was in Los Angeles, next day opponent likely to be a west coast team. There appears to be less of a correlation between last game location in New York or Brooklyn and next day opponent, perhaps because players ``gain'' time when flying east-to-west.} \end{figure}

Our takeaway from the figure above is that indeed there is a correlation between last game location
and next day opponent, in particular this is true for west coast teams. We hypothesize that
this is due to practical limitations on flight schedules. The fact that there is a correlation between last game location and next day opponent increases the motivation for conditioning on spread, which accounts may help account for systematic biases to the extent that some of these next-day opponents are particularly (not) skilled. 
...
\begin{sidewaysfigure}
  \hspace{-55pt}
  \includegraphics[scale=0.7,angle=180]{Party_by_Team_Location}
\end{sidewaysfigure}
