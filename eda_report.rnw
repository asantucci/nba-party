...
tmp <- data[, list(meet.spread = mean(outcome == 'W'), .N), 
            by = list(avg.age = round(avg.age), party, season)][order(avg.age, season, party)]
tmp[order(avg.age, party)]
