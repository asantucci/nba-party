...
data[grepl("^1[01]/", date), year := substr(season, start = 1, stop = 4) %>% as.integer]
...
### 2009-2010
bad.teams <- data.frame(season = '2009_2010',
...
hist(data[, mean(outcome == 'W'), by = list(season, team)][, V1], breaks = seq(0.325, 0.725, by = 0.02), main = "How often are teams meeting the spread?", xlab = "Proportion of games for which a team-year meets the spread")
