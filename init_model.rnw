...
p <- predict(m, newdata = lines[season == 2017 & nhours.lgame <= HOURS & 
                                last.game.loc != team], type = 'response')
table(round(p), lines[season == 2017 & nhours.lgame <= HOURS & 
...
\paragraph{An out of the box machine learning model}
For fun, we try a simple support vector machine. We posit that whether
teams party also depends on how much time they have between games; even
when restricting our data to the twenty-four hour window after teams visit a party city, there is quite a bit of heterogeneity in when yesterday's game finished and when 
today's starts that a non-linear classifier may be able to take advantage of.

<<echo=2:5>>=
require(e1071)
svm.data <- lines[last.game.loc != team, 
                  list(hour = hour(date), last.game.time, party, 
                       loss.season, season, outcome)] %>%
    na.omit

### tune.svm(..., gamma = 2^(-5:5))
...
lm(demeaned.reb.per.min ~ party + lag.chg.pos +
   ...
   data = data[season < 2017 & last.game.loc != team] %>% na.omit) %>%
   summary
@ 
...
we would earn a profit of just less than \$400 over the course of 45 games.
