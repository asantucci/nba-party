...
plot(x = ticks, 
     ...
     main = 'Continuous measure of party by Team Location',
...
mpa <- lm(team.pts.admitted ~ party + lag.chg.pos + log(travel.dist+1) + nhours.lgame, 
   data=nba.lines[last.game.loc!=team] %>% na.omit)
mpo <- update(mpa, team.pts.scored ~ .)
...
#stargazer(mpa1, mpa, mpo, single.row=T)
...
#stargazer(m0, m1, m2, m3, single.row = T)
@ 
