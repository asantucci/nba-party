...
<<>>= 
musicians[, list(average.nmusicians = mean(nmusicians) %>% round(., digits = 1)), by = team][order(average.nmusicians)]
@ 
...
<<echo=-4>>= 
lines[, party := ifelse(ndays.lgame == 1, nmusicians, 0)]

m <- glm(outcome == 'W' ~ party + ndays.lgame + travel.dist + avg.age, 
...
p <- predict(m, newdata = lines[season == 2017 & ndays.lgame == 1 & last.game.loc != team], 
             type = 'response')
