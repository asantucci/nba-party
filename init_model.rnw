...
## load(file = 'tmp_data/nba_musicians.RData')
load(file = 'tmp_data/nba_studios.RData')
...
## nba.lines[, list(date, team, opponent, location, 
...
stargazer(m0, m1, covariate.labels = c('party discrete', 'party continuous', 'lag changes in posession', 'logged travel distance', 'east-west travel direction', 'number hours rest time', 'time of game during day', 'home team effect', 'logged travel distance * east-west', 'Constant'),
...
plot(x = ticks, 
     ...
     ylab = 'Log Avg of Musicians and Sound Recording Studios',
     xlab = '',
     axes = F)
...
stargazer(mp, covariate.labels = c('party placebo', 'lag changes in posession', 'logged travel distance', 'east-west travel direction', 'number hours rest time', 'time of game during day', 'home team effect', 'logged travel distance * east-west', 'Constant'),
          dep.var.labels = 'Meet the Spread')

##################################################
### Table for uncorrelated next day opponent
##################################################
...
##################################################
### Why drinking estabs doesn't work.
##################################################
load(file = 'tmp_data/nba_drinking.RData')
drinking <- copy(dt)
setnames(drinking, 'variable', 'ndrinks')
rm(dt)

tmpm <- musicians[, list(log.nmusicians = mean(log(nmusicians+1)),
                         log.population = mean(log(population))), 
...
plot(x = ticks, 
     ...
     ylab = 'Log Avg of Variable',
     xlab = '', axes = F, pch = 1, ylim = c(0, max(tmp$log.population)))
points(x = ticks, y = tmp$log.ndrinks, pch = 16, col = 'purple')
points(x = ticks, y = tmp$log.population, pch = 18, col = 'orange')
axis(side = 1, at = ticks, labels = gsub('([a-z]{4,}) ', '\\1\n', tmp$team) %>% 
                               gsub('([a-z]{6})[a-z]{1,}', '\\1', .), las = 2, xpd = NA)
axis(side = 2)
legend(x = 22.5, y = 10.25, legend = c('Musicians and sound recording studios',
                                 'Drinking establishments and liquor stores',
                                 'Population'), pch = c(1, 16, 18), col = c('black', 'purple', 'orange'))
text(x = 24.6, y = 10.5, labels = paste("Correlation (alcohol):       ", 
                                       cor(tmp$log.nmusicians, tmp$log.ndrinks) %>% round(., 3)))
text(x = 24.6, y = 11, labels = paste("Correlation (population): ", 
...
## stargazer(m3, covariate.labels = c('Continuous measure of nightlife',
##                                    'Bookmaker\'s odds', 'Home-team effect',
##                                    'Number of rest days', 'Logged travel distance',
##                                    'Weekend', 'Constant'), 
##           dep.var.capation = 'Probability of Winning')
...
b <- lapply(2012:2016, function(yr) {
    m <- glm(I(team.score > opponent.score) ~ party + odds,
             data = mlb.lines[year(date) < yr], family = 'binomial')
    p <- predict(m, newdata = mlb.lines[year(date) == yr], type = 'response')
    idx <- which(p < .45)
    p1 <- mapply(Predict, prediction = p[idx], 
                 vegas = mlb.lines[year(date) == yr, odds][idx],
                 MoreArgs=list(threshold = 0.01))
    b <- mapply(Bet, our.prediction = p1,
                actual.outcome = mlb.lines[year(date) == yr, team.score > opponent.score][idx],
                odds = mlb.lines[year(date) == yr, odds][idx], SIMPLIFY = T)
    cat(paste0(sum(b, na.rm = T), '\n'))
    ### 
#    pdf(paste0('mlb_bets_', yr, '.pdf'))
    betting.days <- mlb.lines[year(date) == yr][idx, date]
    running.prof <- cumsum(ifelse(is.na(b), 0, b))
    plot(x = 1:length(b), y = running.prof, 
         main = paste0('Profits over the ', yr, ' season\n',
    ...
    text(x = which.min(running.prof), y = min(running.prof), 
         labels = round(min(running.prof)), pos = 4)
    return(b)
#    dev.off()
})
dev.off()
...
m <- glm(I(team.score > opponent.score) ~ party + odds,
         data = mlb.lines[year(date) < 2016], family = 'binomial')
p <- predict(m, newdata = mlb.lines[year(date) == 2016], type = 'response')
idx <- which(p < .45)
p1 <- mapply(Predict, prediction = p[idx], 
             vegas = mlb.lines[year(date) == 2016, odds][idx],
             MoreArgs=list(threshold = 0.01))
b <- mapply(Bet, our.prediction = p1,
                actual.outcome = mlb.lines[year(date) == 2016, team.score > opponent.score][idx],
            odds = mlb.lines[year(date) == 2016, odds][idx], SIMPLIFY = T)

### the following plot shows sufficient deviation from house odds to motivate
### an optimized betting strategy.
hist(p- mlb.lines[year(date) == 2016, odds])
