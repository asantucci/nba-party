...
lines[, simple.date := as.Date(date)]
data <- merge(lines, espn, by.x = c('team', 'simple.date'), by.y = c('team', 'date')) ### Careful which obs we keep!
...
lm(I(as.numeric(pm) / min) ~ party, data = data[season < 2017 & last.game.loc != team]) %>% summary
