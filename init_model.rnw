...
load(file = 'tmp_data/studios_and_artists_nba.RData')
...
stargazer(m4, covariate.labels = c('Party placebo',
                                   'Bookmaker\'s odds', 'Home-team effect',
                                   'Number of rest days', 'Logged travel distance',
                                   'Weekend', 'Constant'), 
          dep.var.labels = 'Probability of Winning (MLB)')
...
tmp <- dt[, list(party.continuous = mean(log(nmusicians+1))),
          by = team][order(party.continuous)][!is.na(party.continuous)]
