...
par(mfrow = c(3,2))
lapply(2012:2016, function(yr) {
    m <- glm(I(team.score > opponent.score) ~ party + odds,
             data = lines[year(date) < yr], family = 'binomial')
    p <- predict(m, newdata = lines[year(date) == yr], type = 'response')
    idx <- which(p < .45)
    p1 <- mapply(Predict, prediction = p[idx], 
                 vegas = lines[year(date) == yr, odds][idx],
                 MoreArgs=list(threshold = 0.01))
    b <- mapply(Bet, our.prediction = p1,
                actual.outcome = lines[year(date) == yr, team.score > opponent.score][idx],
                odds = lines[year(date) == yr, odds][idx], SIMPLIFY = T)
    cat(paste0(sum(b, na.rm = T), '\n'))
    ### 
#    pdf(paste0('mlb_bets_', yr, '.pdf'))
    betting.days <- lines[year(date) == yr][idx, date]
