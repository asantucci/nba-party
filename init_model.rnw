...
### This is simply ad-hoc.
musicians[, population := 0.0]
for (i in 1:nrow(musicians))
    set(musicians, i, j = 'population', 
        value = pop[grepl(musicians[i, locality], locality) &
                    grepl(musicians[i, state],    locality) &
                    musicians[i, season] == season, mean(population)])
...
plot(x = ticks, 
     ...
     xlab = '', axes = F, pch = 1, ylim = c(0, max(tmp$log.population)), type = 'l')
...
par(mfcol = c(2, 1))
nba.lines[, list(meet.spread = mean(outcome == 'W')), 
          by = list(season, team)][, hist(meet.spread, 
                                          main = paste('How often do teams meet the spread? (NBA)', 
...
mlb.lines[, list(avg.residual = mean(resid)), by = round(fitted, 3)] %>% 
    plot(., xlab = 'Fitted Probability', ylab = 'Average Residual', 
